 ...
 * @property {?function(exes: Array<string>): string} [exeSelector]
 ...
 * @return {Promise<{opts: LaunchOptions, udataDir: ?string, suppliedUdata: boolean}>}
 */
async function ensureArgs(options) {
  ...
  if (opts.userDataDir) {
    if (suppliedUdata) {
      opts.args = opts.args.filter(arg => !arg.startsWith('--user-data-dir'))
    }
    suppliedUdata = true
  ...
  if (!suppliedUdata) {
    udataDir = await makeTempUDataDir(opts.userDataDirPrefix)
  }
 ...
 *
 * @param {?LaunchOptions} [options]
 * @return {Promise<{browserWSEndpoint: string, chromeProcess: ChildProcess, closeBrowser: (function(): Promise<void>), waitForChromeToClose: Promise<void>}>}
 */
module.exports = async function launch(options) {
  const { opts, udataDir, suppliedUdata } = await ensureArgs(options)
  ...
  try {
    browserWSEndpoint = await waitForWSEndpoint(
      chromeProcess,
      typeof opts.launchTimeout === 'number' ? opts.launchTimeout : 30000
    )
  } catch (error) {
    killChrome()
    throw error

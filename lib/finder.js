...
async function findChromeLinux() {
  ...
  for (i = 0; i < exePriorities.length; ++i) {
    ...
    exe = await whichSymLinkResolved(priority.which)
...
async function testExe(exe, exePriorities, seen, found, defaultWeight) {
  ...
  let priority
  let useDefault = true
  const actualExePath = await maybeResolveLink(exe)
  for (let nthPriority = 0; nthPriority < exePriorities.length; ++nthPriority) {
    priority = exePriorities[nthPriority]
    if (
      !seen.has(actualExePath) &&
      priority.regex.test(actualExePath) &&
      (await bingo(actualExePath))
    ) {
      seen.add(actualExePath)
      found.push({ exe: actualExePath, weight: priority.weight })
      useDefault = false
      break
    }
  }
  if (useDefault && defaultWeight && !seen.has(actualExePath)) {
    seen.add(actualExePath)
    found.push({ exe: actualExePath, weight: defaultWeight })
  }
}

async function maybeResolveLink(exe) {
  if (!exe) return exe
  try {
    const stat = await fs.lstat(exe)
    if (stat.isSymbolicLink()) return fs.realpath(exe)
    return exe
  } catch (error) {
    return exe
  }
...
function whichSymLinkResolved(executable) {
  return which(executable).then(maybeResolveLink)
}
